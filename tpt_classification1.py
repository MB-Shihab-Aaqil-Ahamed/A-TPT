...
#         # Number of data points and weight
#         n = len(labels)
...
#         # Plot vertical dashed lines for Accuracy and Avg. Confidence
...
#         # Add text labels for Accuracy and Confidence
#         if acc > conf:
...
#         plot_filename = os.path.join('plots', f"confidence_histogram_{args.test_sets}.png")
...
def SCE_Loss(num_bins, predictions, confidences, correct, n_class):
    #ipdb.set_trace()
    ...
    for idx in range(num_bins):
        ...
        sce_loss += (temp_abs*bin_num_sample[idx])/len(predictions)

    return sce_loss/n_class, bin_accuracy, bin_confidence, bin_num_sample
...
def Calculator(result_dict):
    ...
    # Replace ECE with SCE
    sce_data = SCE_Loss(20, list_prediction, list_max_confidence, list_correct, n_class)
    acc = sum(list_correct) / len(list_correct)
    ...
    # confidence_histogram.plot(list_prediction, list_label, n_bins=20, title=f"Confidence Histogram A-TPT-{args.test_sets}")

    print('acc: ', acc * 100)
    print('sce: ', sce_data[0] * 100)  # SCE loss
          
    # ReliabiltyDiagram(ece_data[1], n_bins=20, title=f"Reliability Diagram A-TPT-{args.test_sets}")
          
    return acc*100, sce_data[0]*100
...
if __name__ == '__main__':
    ...
    acc, sce = Calculator(result_dict)