...
def test_time_tuning(model, inputs, optimizer, scaler, args):
    ...
    for j in range(args.tta_steps):
        ...
        if 'atpt' in args.run_type:
            ...
            print(model.get_text_features().shape)
    ...
    with torch.no_grad():
        ...
        df = pd.DataFrame({
            'Index': [next_index],
            'MeanCosineSimilarity': [mean_Wwt]
        })

        df.to_csv(csv_path, mode='a', header=write_header, index=False)

        print(f"[{next_index}] Saved mean cosine similarity to {csv_path}: {mean_Wwt}")
