...
def save_embeddings_metrics_to_csv(ctx_init, text_features, acc, ece, ad, csv_path):

    _, embed_dim = text_features.shape
    header = ['ctx_init', 'accuracy', 'ECE', 'AD', 'feature_index'] + [f'feature_dim_{i}' for i in range(embed_dim)]

    with open(csv_path, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        for idx, feature in enumerate(text_features):
            writer.writerow([ctx_init, acc, ece, ad, idx] + feature.tolist())

def save_tuned_embeddings_metrics_to_csv(text_features, acc, ece, ad, lambda_term, csv_path):
    _, embed_dim = text_features.shape
    header = ['lambda', 'accuracy', 'ECE', 'AD', 'feature_index'] + \
             [f'feature_dim_{i}' for i in range(embed_dim)]

    with open(csv_path, mode='w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)
        for idx, feature in enumerate(text_features):
            writer.writerow([lambda_term, acc, ece, ad, idx] + feature.tolist())
...
def ReliabiltyDiagram(bin_acc, n_bins=20, title="None"):
    ...
    plot_filename = os.path.join('plots', f"reliability_diagram_{args.test_sets}.png")
...
def test_time_adapt_eval(val_loader, model, model_state, optimizer, optim_state, scaler, args, result_dict):
    ...
    # csv_path = os.path.join("saved_csv", f"{args.ctx_init}_embeddings_metrics.csv")
    os.makedirs("saved_tuned_csv", exist_ok=True)
    csv_path = os.path.join("saved_tuned_csv", f"tuned_lambda_{args.lambda_term}.csv")

    text_features = model.get_text_features().detach().cpu().numpy()
    acc, ece = Calculator(result_dict)
    ad = model.ang_norm_mean

    # save_embeddings_metrics_to_csv(args.ctx_init, text_features, acc, ece, ad, csv_path)
    save_tuned_embeddings_metrics_to_csv(text_features, acc, ece, ad, args.lambda_term, csv_path)
