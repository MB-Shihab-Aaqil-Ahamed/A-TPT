...
def ConfidenceHistogram(confidences, accuracies, n_bins=20, title=None):
    ...
    plot_filename = os.path.join('figs', f"confidence_histogram_{args.test_sets}.png")
...
def ReliabiltyDiagram(bin_acc, n_bins=20, title="None"):
    ...
    plot_filename = os.path.join('figs', f"reliability_diagram_{args.test_sets}.png")
    plt.savefig(plot_filename)
    print(f"plot saved to {plot_filename}")
...
def Calculator(result_dict):
    ...
    ReliabiltyDiagram(ece_data[1], n_bins=20, title=f"Reliability Diagram L-TPT-{args.test_sets}")

    ConfidenceHistogram(list_max_confidence, list_correct, n_bins=20, title=f"Confidence Histogram L-TPT-{args.test_sets}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tau_term' , type=float, default=0.0, help='tau for ang_norm_mean')
